...
public class StateWaitINS implements IAFState {
    ...
    public String doAction(AbstractAF af, Object instance, ArrayList<EquinoxRawData> rawDatas) {
        ...
        for (EquinoxRawData r : rawDatas) {
            ...
            if (r.getRawEventType().equals(AFEvent.IncomingHttpRes)) {
                SmoiStatAlarm.incrementStats(af, smoiIns.getPage(), StatAlarm.INGateway_Receive_INGW_INS_$s_Response, smoiIns);
                ...
                SmoiStatAlarm.incrementStats(af, smoiIns.getPage(), StatAlarm.INGateway_Send_HTTP_Response_$s, smoiIns);
                nextState = AFState.IDLE;
            } else if (r.getRawEventType().equals(AFEvent.IncomingEquinox_Reject)) {
            	int maxRetry = smoiIns.getResourceNameStandbyMaxRetry(Conf.resourceNameIns_Standby_MaxRetry);
            	if(maxRetry>0){       
            		 //TODO Standby_MaxRetry
            		ec02.utils.Log.d("Retry: "+Conf.resourceNameIns_Standby_MaxRetry+" maxRetry: "+maxRetry);
             		maxRetry -= 1;
                    smoiIns.setResourceNameStandbyMaxRetry(Conf.resourceNameIns_Standby_MaxRetry,maxRetry);
                    ...
                    SendMessage.SMOI(outputMsg, af, myAppData,Conf.resourceNameIns_Standby);
	            	SmoiStatAlarm.incrementStats(af, smoiIns.getPage(), StatAlarm.INGateway_Receive_INGW_INS_$s_Request_Reject, smoiIns);
                ...
                SmoiStatAlarm.incrementStats(af, smoiIns.getPage(), StatAlarm.INGateway_Receive_INGW_INS_$s_Request_Abort, smoiIns);
                outputMsg = mapEquinoxAbortCmd();
                SendMessage.Client(outputMsg, af, myAppData);
                SmoiStatAlarm.incrementStats(af, smoiIns.getPage(), StatAlarm.INGateway_Send_HTTP_$s_Response_Error, smoiIns);
                nextState = AFState.IDLE;
            } else if (r.getRawEventType().equals(AFEvent.IncomingEquinox_Timeout)) {
                SmoiStatAlarm.incrementStats(af, smoiIns.getPage(), StatAlarm.INGateway_Receive_INGW_INS_$s_Request_Timeout, smoiIns);
                outputMsg = mapEquinoxTimoutCmd();
                SendMessage.Client(outputMsg, af, myAppData);
                SmoiStatAlarm.incrementStats(af, smoiIns.getPage(), StatAlarm.INGateway_Send_HTTP_$s_Response_Error, smoiIns);
                nextState = AFState.IDLE;
            } else if (r.getRawEventType().equals(AFEvent.IncomingEquinox_Error)) {
                SmoiStatAlarm.incrementStats(af, smoiIns.getPage(), StatAlarm.INGateway_Receive_INGW_INS_$s_Request_Error, smoiIns);
    ...
    private String mapEquinoxRejectCmd() {
        String msg;
        String res = EResponseCode.INGW_INS_Reject.getCode();
        String desc = EResponseCode.INGW_INS_Reject.getDesc();
    ...
    private String mapEquinoxErrorCmd() {
        String msg;
        String res = EResponseCode.INGW_INS_Error.getCode();
        String desc = EResponseCode.INGW_INS_Error.getDesc();
        msg = "<vcrr><res>" + res + "</res><desc>" + desc + "</desc></vcrr>";
        return msg;
    }
    ...
    private String mapEquinoxAbortCmd() {
        String msg;
        String res = EResponseCode.INGW_INS_Abort.getCode();
        String desc = EResponseCode.INGW_INS_Abort.getDesc();
        msg = "<vcrr><res>" + res + "</res><desc>" + desc + "</desc></vcrr>";
        return msg;
    }
    ...
    private String mapEquinoxTimoutCmd() {
        String msg;
        String res = EResponseCode.INGW_INS_Timeout.getCode();
        String desc = EResponseCode.INGW_INS_Timeout.getDesc();
        msg = "<vcrr><res>" + res + "</res><desc>" + desc + "</desc></vcrr>";
        return msg;
    }
}
